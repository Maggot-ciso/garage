...
/** Which vehicles a light can actually appear on. */
export type LightFits = 'both' | 'car' | 'motorcycle'
...
export interface DashboardLight {
  ...
  severity: Severity
  /**
   * Showing a car's airbag or door-ajar lamp to a scooter rider is noise, and
   * hiding the neutral lamp from them is a gap. Default 'both' where the light
   * genuinely appears on either.
   */
  fits: LightFits
  meaning: string
  whatToDo: string
...
export const DASHBOARD_LIGHTS: DashboardLight[] = [
  ...
  },
  {
    ...
    severity: 'note',
    fits: 'motorcycle',
    meaning: 'The gearbox is in neutral. Normal, and the light most riders check before thumbing the starter.',
    whatToDo: 'Nothing. If it stays lit in gear, or never lights in neutral, suspect the neutral switch rather than the gearbox.',
    svg: '<circle cx="12" cy="12" r="8.5" fill="none" stroke="currentColor" stroke-width="1.6"/><path d="M9.3 15.5v-7l5.4 7v-7" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/>',
  },
  {
    id: 'side-stand',
    name: 'Side stand down',
    colour: 'amber',
    severity: 'soon',
    fits: 'motorcycle',
    meaning: 'The side stand is down. Most bikes cut the engine if you select a gear with the stand down — and riding off with it down catches it on the first left-hander.',
    whatToDo: 'Put the stand up before pulling away. If it stays lit with the stand up, the stand switch is likely dirty or failing.',
    svg: '<path d="M8 4v11" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><path d="M8 15l6 4" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><path d="M4 20h16" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/>',
  },
  {
    id: 'fi-fault',
    name: 'FI / fuel injection',
    colour: 'amber',
    severity: 'soon',
    fits: 'motorcycle',
    meaning: 'The fuel-injection ECU has logged a fault. On a bike this lamp doubles as the fault-code reader: a steady light means a stored fault, a flashing one is spelling out a code.',
    whatToDo: 'If it is flashing, count the pattern — the fault-code reader on this screen turns it into a number. Rideable if it runs normally, but get it read before a long trip.',
    svg: '<path d="M7 20V9a2 2 0 0 1 2-2h3a2 2 0 0 1 2 2v11" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"/><path d="M5 20h11" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/><path d="M14 11h2.5a1.5 1.5 0 0 1 1.5 1.5V16a1.5 1.5 0 0 0 1.5 1.5" fill="none" stroke="currentColor" stroke-width="1.4"/><path d="M9 4.5l-1 2.5h3l-1 2.5" fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>',
  },
]
