  ...
  if (view.mode === 'edit') {
    return (
      <CarForm
        ...
        onDelete={async () => {
          if (window.confirm(`Delete ${view.car.make} ${view.car.model}? This cannot be undone.`)) {
...
function BackupNudge() {
  const nudge = useLiveQuery(async () => {
    ...
    if (days === null) return 'You haven’t exported a backup yet'
    if (days > NUDGE_AFTER_DAYS) return `Last backup was ${days} days ago`
    return null
  }, [])
