    ...
    <div className="flex flex-col gap-4 pb-2">
      ...
      <section className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          ...
          >
            Logbook <ChevronRight className="h-4 w-4" strokeWidth={2} aria-hidden />
          </button>
        </div>
        {entries.length === 0 ? (
          <p className="faint text-sm">Nothing logged yet — add a fill-up in the Logbook.</p>
        ) : (
    ...
    <section className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="link-accent flex items-center"
        >
          Full chat <ChevronRight className="h-4 w-4" strokeWidth={2} aria-hidden />
        </button>
      </div>

      {!apiKey ? (
        <p className="faint text-sm">Add your API key in Settings to chat about this car.</p>
      ) : (
        <div className="card flex flex-col gap-2 p-3">
          {messages.length === 0 && !busy && (
            <p className="faint text-sm">
              Anything about your {car.model} — symptoms, parts, service planning.
            </p>
          )}
          {messages.length > 0 && (
          ...
          {busy && !pending && <p className="faint text-sm">Thinking…</p>}
