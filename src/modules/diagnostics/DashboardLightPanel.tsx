...
export function DashboardLightPanel() {
      ...
      <section className="flex flex-col gap-2">
        ...
        <div className="card flex flex-col gap-2 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-bold tracking-tight">{light.name}</div>
              <div className="muted text-sm">{SEVERITY_LABEL[light.severity]}</div>
            </div>
          </div>
          <p className="text-sm">{light.meaning}</p>
          <p className="text-sm">
            <span className="font-medium">What to do: </span>
            {light.whatToDo}
          </p>
        </div>
        <p className="faint text-xs">Guidance only — never a diagnosis. Confirm with a mechanic.</p>
      </section>
    )
  }
    ...
    return (
      <section className="flex flex-col gap-2">
        <button
          ...
          className="link-accent flex items-center gap-1 self-start"
        >
          <ChevronLeft className="h-4 w-4" strokeWidth={2} aria-hidden /> Colours
        </button>
        <h2 className="section-title flex items-center gap-2">
          <span className={`h-3 w-3 rounded-full ${COLOUR_SWATCH[colour]}`} />
          {COLOUR_LABEL[colour]} lights
        </h2>
        <ul className="grid grid-cols-3 gap-2">
          {lightsOfColour(colour).map((l) => (
            <li key={l.id}>
              <button
                type="button"
                onClick={() => setLight(l)}
  ...
  return (
    <section className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        <h2 className="section-title">Warning light</h2>
      </div>
      <p className="faint text-sm">A light came on? Pick its colour.</p>
        ...
        {COLOUR_ORDER.map((c) => (
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block font-medium">{COLOUR_LABEL[c]}</span>
              <span className="faint text-sm">{COLOUR_HINT[c]}</span>
            </span>
      ...
      <p className="faint text-xs flex items-center gap-1">
        ...
        Common lights only — your manual covers the rest.
      </p>
    </section>
  )
}
